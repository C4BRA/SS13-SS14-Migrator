...
exports.CSharpEmitter = void 0;
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
const builtinMappings_js_1 = require("./builtinMappings.js");
class CSharpEmitter {
    tempCounter = 0;
    currentProcName = '';
    ...
    emitCSharpSystems(irMap, outputServerDir, globals = [], symbols) {
     ...
     *  memory (item 66). */
    emitCSharpSystemsFile(irMap, outputServerDir, globals = [], symbols) {
        if (!fs.existsSync(outputServerDir)) {
            fs.mkdirSync(outputServerDir, { recursive: true });
        }
        this.symbols = symbols ?? null;
    ...
    generateProcsCSFile(irMap, globals, filePath) {
...
`);
    ...
    emitProcsCS(irMap, globals, sinks) {
        ...
        for (const [pathKey, irType] of irMap.entries()) {
            ...
            for (const [procName, procNode] of irType.procs.entries()) {
                ...
                const paramNames = procNode.args && procNode.args.length > 0
                    ? `, new[] { ${procNode.args.map((a) => `"${this.escapeString(a.name)}"`).join(', ')} }`
                    : '';
                registrations.push(`            ProcRegistry.Register("${this.escapeString(pathKey)}", "${this.escapeString(procName)}", Proc_${className}_${csharpProcName}${paramNames});`);
            }
        }
...
`;
    ...
    transpileExpression(node) {
        switch (node.type) {
            ...
            case 'new':
                ...
                return `(await DMNew(${this.globalsMode ? 'null' : 'comp'}, "${this.normalizeTypePath(node.typePath)}"${node.arguments.length > 0 ? ', ' + node.arguments.map((a) => this.transpileExpression(a)).join(', ') : ''}))`;
            case 'ternary':
            ...
            case 'assignment':
                ...
                if (this.globalsMode) {
                    return `(await GlobalVars.Set("${node.target}", ${this.transpileExpression(node.value)}))`;
                }
                return `comp.SetVar("${node.target}", ${this.transpileExpression(node.value)})`;
            case 'property_assignment': {
                // GLOB.x = v writes through the generated GlobalVars registry.
                const paTarget = node.target;
                if (paTarget?.type === 'variable' && paTarget.name === 'GLOB') {
                    return `(await GlobalVars.Set("${node.property}", ${this.transpileExpression(node.value)}))`;
                }
                return `(${this.transpileExpression(paTarget)}).AsDatum()?.SetVar("${node.property}", ${this.transpileExpression(node.value)}) ?? DMValue.Null`;
            }
            ...
            case 'list':
                ...
                if (node.elements.some((e) => e.type === 'assoc_pair')) {
                    const kv = node.elements.map((e) => e.type === 'assoc_pair'
                        ? `${this.transpileExpression(e.key)}, ${this.transpileExpression(e.value)}`
                        : this.transpileExpression(e)).join(', ');
                    return `DMRuntimeHelpers.MakeListAssoc(${kv})`;
                }
                return `DMRuntimeHelpers.MakeList(${node.elements.map((e) => this.transpileExpression(e)).join(', ')})`;
            case 'range':
        ...
        switch (node.literalType) {
            ...
            case 'number':
                if (node.value === 'Infinity')
                    return 'DMValue.FromNumber(double.PositiveInfinity)';
                if (node.value === '-Infinity')
                    return 'DMValue.FromNumber(double.NegativeInfinity)';
                if (node.value === 'NaN')
                    return 'DMValue.FromNumber(double.NaN)';
    ...
    transpileVariable(node) {
        ...
        // world datum.
        if (node.name === 'src')
            return this.globalsMode ? 'DMValue.Null' : 'DMValue.FromDatum(comp)';
        if (node.name === 'usr')
            return 'DMRuntimeHelpers.CurrentUsr';
        if (node.name === 'world')
            return 'DMRuntimeHelpers.WorldValue';
        if (node.name === 'args')
            return this.globalsMode ? 'DMValue.Null' : 'DMValue.FromList(__dmArgs)';
        // Global initializers have no proc scope: a bare identifier references
        // another global (there is no comp to read from).
        if (this.globalsMode)
            return `await GlobalVars.Get("${node.name}")`;
    ...
    referencesIdentifier(node, name) {
        ...
        if (Array.isArray(node))
            return node.some(n => this.referencesIdentifier(n, name));
        for (const key of Object.keys(node)) {
            if (this.referencesIdentifier(node[key], name))
                return true;
        }
        return false;
    }
        ...
        switch (node.operator) {
            ...
            case '<<': {
                ...
                if ((t.type === 'variable' && (t.name === 'world' || t.name === 'usr')) ||
                    (t.type === 'property' && t.target?.name === 'world')) {
    ...
    transpileCall(node) {
        ...
        const hasArglist = node.arguments.some((a) => a.type === 'call' && a.name === 'arglist');
        const args = hasArglist
            ? `DMRuntimeHelpers.DMArgsConcat(${node.arguments.map((a) => a.type === 'call' && a.name === 'arglist'
                ? `DMRuntimeHelpers.DMArgList(${this.transpileExpression(a.arguments[0])})`
                : `DMRuntimeHelpers.DMArgList(${this.transpileExpression(a)})`).join(', ')})`
            : node.arguments.map((a) => this.transpileExpression(a)).join(', ');
        // DM associative literal: list("a" = 1, "b" = 2) — arguments are
        // key/value pairs emitted through MakeListAssoc (plain args are values).
        if ((node.name === 'list' || node.name === 'alist') && node.arguments.some((a) => a.type === 'assoc_pair')) {
            const kv = node.arguments.map((a) => a.type === 'assoc_pair'
                ? `${this.transpileExpression(a.key)}, ${this.transpileExpression(a.value)}`
                : this.transpileExpression(a)).join(', ');
            return `DMRuntimeHelpers.MakeListAssoc(${kv})`;
        }
        ...
        if (node.name === '' && node.target) {
            const refArgs = node.arguments.map((a) => this.transpileExpression(a)).join(', ');
        ...
        const builtin = (0, builtinMappings_js_1.transpileBuiltinCall)(node.name.toLowerCase(), args);
        if (builtin !== null) {
            return builtin;
        }
     ...
     *  (CS0102). */
    pathClassNameMap = new Map();
    usedClassNames = new Set();
    pathToClassName(dmPath) {
        ...
        if (this.usedClassNames.has(name)) {
            let suffix = 2;
            while (this.usedClassNames.has(`${name}_${suffix}`))
                suffix++;
            name = `${name}_${suffix}`;
        }
    ...
    nextProcMemberName(procName, used) {
    ...
    resolveAndWarn(name, isMethodCall) {
        if (!this.symbols || name === '' || name === '..')
            return;
        const lname = name.toLowerCase();
        if (this.warnedCalls.has(lname))
            return;
...
exports.CSharpEmitter = CSharpEmitter;
