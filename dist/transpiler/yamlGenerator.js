...
class YAMLGenerator {
    ...
    static BASE_PARENT_STUBS = ['BaseItem', 'BaseFloor', 'BaseWall', 'BaseMobDummy'];
    generateYAMLPrototypes(irMap, outputDir) {
        ...
        for (const proto of prototypes) {
            ...
            emit(`  id: ${proto.id}\n`);
            if (proto.parent)
                emit(`  parent: ${proto.parent}\n`);
            if (proto.name)
                emit(`  name: ${this.yamlScalar(proto.name, true)}\n`);
            if (proto.description)
                emit(`  description: ${this.yamlScalar(proto.description, true)}\n`);
    ...
    parentIdFor(irType, pathKey, idMap) {
        if (pathKey === '/turf') {
    ...
    serializeProps(obj, indent, skipTopLevelType = false) {
        let out = '';
    ...
    static YAML_PLAIN_UNSAFE = /^(yes|no|on|off|true|false|null|~)$|^[-+]?[0-9]|^0x[0-9a-fA-F]|^\.(inf|nan)/i;
    yamlScalar(v, forceQuote = false) {
