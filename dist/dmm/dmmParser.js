...
class DMMParser {
    ...
    parseDefinition(key, body, mapData) {
    ...
    parseEntry(text) {
        ...
        if (openIdx < 0 || !trimmed.endsWith('}'))
            return { path: trimmed, attrs: undefined };
        let depth = 0;
        let inString = false;
        let closeIdx = -1;
    ...
    decodeLine(rawLine, keyLen, sortedKeys) {
    ...
    buildSectionGrid(section, keyLen, sortedKeys, warnings) {
        const cells = [];
    ...
    mergeSections(z, sections, mapData) {
