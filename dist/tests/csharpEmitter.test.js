...
function transpileProc(dmCode) {
    const parser = new dmParser_js_1.DMParser(new dmLexer_js_1.DMLexer(dmCode).tokenize());
    const nodes = parser.parse();
    assert(parser.diagnostics.errors.length === 0, `No parse errors for: ${dmCode.trim().split('\n')[1] || dmCode}`);
    const ir = new dmIRGenerator_js_1.DMIRGenerator().generateIR(nodes);
    return new csharpEmitter_js_1.CSharpEmitter().generateProcsCS(ir, parser.globalVars);
}
...
`);
    ...
    const rangeTokens = new dmLexer_js_1.DMLexer('1..5').tokenize();
    assert(rangeTokens[0].type === dmLexer_js_1.TokenType.Number && rangeTokens[0].value === '1', 'Lexer: 1..5 starts with Number 1');
    assert(rangeTokens[1].type === dmLexer_js_1.TokenType.Operator && rangeTokens[1].value === '..', 'Lexer: 1..5 emits .. operator');
    const bomTokens = new dmLexer_js_1.DMLexer('\uFEFF/obj/foo').tokenize();
    assert(bomTokens[0].type === dmLexer_js_1.TokenType.TypePath && bomTokens[0].value === '/obj/foo', 'Lexer: UTF-8 BOM stripped');
    // Test 16: Lexer — unterminated block comment produces a diagnostic
    const badComment = new dmLexer_js_1.DMLexer('/obj/foo\n/* comment never closed');
    badComment.tokenize();
    ...
    const opToks = new dmLexer_js_1.DMLexer('a % b ~ c @ d $ e ?. f').tokenize();
    for (const [value, msg] of [['%', 'Lexer: % operator'], ['~', 'Lexer: ~ operator'], ['@', 'Lexer: @ operator'], ['$', 'Lexer: $ operator']]) {
        assert(opToks.some(t => t.type === dmLexer_js_1.TokenType.Operator && t.value === value), msg);
    }
    assert(opToks.some(t => t.type === dmLexer_js_1.TokenType.Operator && t.value === '?.'), 'Lexer: ?. null-conditional operator');
    // Test 18: Lexer — backslash line continuation is skipped
    const cont = new dmLexer_js_1.DMLexer('/obj/foo\n    var/x = 1 + \\\n        2\n').tokenize();
