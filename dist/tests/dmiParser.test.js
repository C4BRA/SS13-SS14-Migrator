...
const dmiParser_js_1 = require("../dmi/dmiParser.js");
...
async function runDMIParserTests() {
    ...
    const parser = new dmiParser_js_1.DMIParser();
...
# END DMI`;
        ...
        assert(bdResult.warnings[0].includes('delay list length'), 'Delay/frames mismatch warning message is descriptive');
...
const ihdr = (w, h, bd = 8, ct = 6, interlace = 0) => {
...
const chunk = (type, data) => {
    ...
    c.writeUInt32BE((0, pngCodec_js_1.crc32)(Buffer.concat([Buffer.from(type), data])), 0);
...
assertThrows(() => (0, pngCodec_js_1.decodePNG)(Buffer.concat([sig.slice(0, 4), Buffer.from([0, 0, 0, 0]), chunk('IHDR', ihdr(1, 1))])), 'PNG signature fully validated (8 bytes)');
// Absurd dimensions rejected before any allocation (would previously hang)
assertThrows(() => (0, pngCodec_js_1.decodePNG)(Buffer.concat([sig, chunk('IHDR', ihdr(100000, 100000))])), 'Huge PNG dimensions rejected');
// Zero dimensions rejected
assertThrows(() => (0, pngCodec_js_1.decodePNG)(Buffer.concat([sig, chunk('IHDR', ihdr(0, 1))])), 'Zero PNG width rejected');
// Invalid color type (1) rejected
assertThrows(() => (0, pngCodec_js_1.decodePNG)(Buffer.concat([sig, chunk('IHDR', ihdr(1, 1, 8, 1))])), 'Invalid PNG color type rejected');
// Invalid bit depth for RGBA (bit depth 4 with colorType 6) rejected
assertThrows(() => (0, pngCodec_js_1.decodePNG)(Buffer.concat([sig, chunk('IHDR', ihdr(1, 1, 4, 6))])), 'Invalid PNG bit depth rejected');
// Interlaced PNGs rejected (Adam7 unsupported)
assertThrows(() => (0, pngCodec_js_1.decodePNG)(Buffer.concat([sig, chunk('IHDR', ihdr(1, 1, 8, 6, 1))])), 'Interlaced PNG rejected');
...
assertThrows(() => (0, pngCodec_js_1.decodePNG)(lying), 'Chunk length beyond file bounds rejected');
...
assertThrows(() => (0, pngCodec_js_1.decodePNG)(shortPng), 'Scanline stream length mismatch rejected');
...
px[2] = 30;
px[3] = 255;
const roundTrip = (0, pngCodec_js_1.decodePNG)((0, pngCodec_js_1.encodePNG)({ width: 2, height: 2, rgba: px }));
...
const bd1 = (0, pngCodec_js_1.decodePNG)(bd1png);
