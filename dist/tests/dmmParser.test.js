...
const dmmParser_js_1 = require("../dmm/dmmParser.js");
...
async function runDMMParserTests() {
    ...
    const parser = new dmmParser_js_1.DMMParser();
...
"}`;
    ...
    assert(orphanResult.warnings.some(w => w.includes('undefined tile key "aaanone"')), 'Undefined grid key is flagged');
    assert(orphanResult.warnings.some(w => w.includes('never used in any grid')), 'Orphan (unused) definition is flagged');
...
"}`;
    ...
    assert(raggedResult.warnings.some(w => w.includes('must be rectangular')), 'Non-rectangular grid is flagged');
