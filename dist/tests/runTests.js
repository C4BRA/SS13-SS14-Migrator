...
const fidelityAudit_js_1 = require("../audit/fidelityAudit.js");
...
/obj/item/weapon/sword
    ...
    proc/activate(power)
        ...
        return power + x
`;
    const lexer = new dmLexer_js_1.DMLexer(sampleDM);
    const tokens = lexer.tokenize();
    assert(tokens.length > 0, "Lexer produced tokens");
    const typePathToken = tokens.find(t => t.type === dmLexer_js_1.TokenType.TypePath);
    ...
    // Test 2: DM Parser
    const parser = new dmParser_js_1.DMParser(tokens);
    ...
    const rootProcTokens = new dmLexer_js_1.DMLexer(rootProcDM).tokenize();
    const rootProcNodes = new dmParser_js_1.DMParser(rootProcTokens).parse();
    ...
    const staticTokens = new dmLexer_js_1.DMLexer(staticDM).tokenize();
    const staticNodes = new dmParser_js_1.DMParser(staticTokens).parse();
    ...
    const fileA = new dmParser_js_1.DMParser(new dmLexer_js_1.DMLexer(`/obj/item/foo\n    var/value = 1`).tokenize()).parse();
    const fileB = new dmParser_js_1.DMParser(new dmLexer_js_1.DMLexer(`/obj/item/foo/proc/run()\n    return value`).tokenize()).parse();
    ...
    const slashDM = new dmParser_js_1.DMParser(new dmLexer_js_1.DMLexer(`/obj/\n    var/flag = 0\n/obj/item/foo/proc/run()\n    return flag`).tokenize()).parse();
    ...
    const globalsParser = new dmParser_js_1.DMParser(new dmLexer_js_1.DMLexer(globalsDM).tokenize());
    const globalsNodes = globalsParser.parse();
    const globalsIR = irGen.generateIR(globalsNodes);
    const globalsEmitter = new csharpEmitter_js_1.CSharpEmitter();
    ...
    const harnessCounters = (0, fidelityAudit_js_1.runAudit)(fixtureDir, 'fixture').counters;
...
# END DMI`;
        ...
        if (fs.existsSync(unzippedInputDir))
            fs.rmSync(unzippedInputDir, { recursive: true, force: true });
        if (fs.existsSync(tmpZipOutputDir))
            fs.rmSync(tmpZipOutputDir, { recursive: true, force: true });
