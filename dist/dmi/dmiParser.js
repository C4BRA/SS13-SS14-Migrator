    ...
    parseDMIText(text) {
        ...
        for (const state of states) {
            if (state.delay && state.delay.length !== state.frames * state.dirs) {
                warnings.push(`state '${state.name}': delay list length ${state.delay.length} does not match frames(${state.frames}) * dirs(${state.dirs})`);
            }
        }
        return {
            version,
