...
class GUIServer {
    ...
    isAuthorized(req) {
        ...
        const hostOk = host === `localhost:${this.port}` || host === `127.0.0.1:${this.port}` ||
            host === 'localhost' || host === '127.0.0.1';
        if (!hostOk)
            return false;
        const origin = req.headers.origin;
        if (origin) {
            const originOk = origin === `http://localhost:${this.port}` || origin === `http://127.0.0.1:${this.port}`;
            if (!originOk)
                return false;
        }
    ...
    static validateOutputPath(outputDirPath) {
        if (!outputDirPath)
            return null;
        if (outputDirPath.includes('\0'))
            return null;
        ...
        if (expanded === '~')
            ...
            expanded = path.join(homeRoot, expanded.slice(2));
        ...
        if (realAncestor === null)
            return null;
        if (realAncestor !== realHome && !realAncestor.startsWith(realHome + path.sep))
            return null;
        return resolved;
    }
    static MAX_UPLOAD_BYTES = 2 * 1024 * 1024 * 1024; // 2 GiB
    static MAX_ZIP_ENTRIES = 50000;
    static MAX_ZIP_UNCOMPRESSED = 4 * 1024 * 1024 * 1024; // 4 GiB
    async handleConvertRequest(req, res) {
        const chunks = [];
        let totalBytes = 0;
        let oversized = false;
        ...
        await new Promise(resolve => {
            req.on('data', chunk => {
                ...
                if (totalBytes > GUIServer.MAX_UPLOAD_BYTES)
                    oversized = true;
                if (!oversized)
                    chunks.push(chunk);
            });
    ...
    async processConvertBody(req, res, chunks, totalBytes, oversized) {
